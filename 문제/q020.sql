--문20
--//	문19를 소수점을 반올림 처리             select no,name,round(weeklywage/7) 주급 from tottenham_squad;				참고. p.156
--      문19를 소수점을 버림 처리               select no,name,trunc(weeklywage/7) 주급 from tottenham_squad;
...
select no,name,round(weeklywage/7) 주급 from tottenham_squad;                    -- 문19의 소수점을 반올림 처리
