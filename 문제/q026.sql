--문26
--      select no,name,nvl(injury,'n') from tottenham_squad;		            참고 p.167
--      select no,name,nvl(injury,'출전가능') as 부상여부 from tottenham_squad;
...
--select * from tottenham_squad where name like '%손%' union select * from tottenham_squad where name like '%탄%';  --문24  선수단 정보 중 이름에 '손' 이 들어간거 출력, 선수단 정보 중 이름에 '탄' 가 들어간거 출력 위 둘을 합쳐서 출력하기

select no,name,nvl(injury,'n') from tottenham_squad;
select no,name,nvl(injury,'출전가능') as 부상여부 from tottenham_squad;
