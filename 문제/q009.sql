--문9
--계속	번호가 6이상이면서 이름에 '손' 이 들어간 선수들만 출력	
--ex.	select * from tottenham_squad where no >= 6 and name like '%손%';	참고 p.111			
...
select * from tottenham_squad where no >= 6 and name like '%손%';                --번호가 6이상이면서 이름에 '손' 이 들어간 선수들만 출력	
