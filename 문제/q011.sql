--문11
--계속	.선수단 선수 목록을 no 순으로 정렬해서 보기 
--select * from tottenham_squad order by no;
--select * from tottenham_squad order by no asc;
--select * from tottenham_squad order by no desc;
...
--update tottenham_squad set birth = '1992.07.08' where no = 7;                    --문10 손흥민 선수의 생일 정보를 입력
select * from tottenham_squad order by no;                                         --선수단 선수 목록을 no 순으로 정렬해서 보기 (기본 default 값)
